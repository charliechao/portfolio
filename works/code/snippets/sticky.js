...
function deleteSticky(e) {
	...
	localStorage.removeItem(key); //pro 3
	var stickiesArray = getStickiesArray();
	if (stickiesArray) {
		for (var i = 0; i < stickiesArray.length; i++) {
		...
		localStorage.setItem("stickiesArray", JSON.stringify(stickiesArray)); //pro 4
...
function addStickyToDOM(key, value) {
	...
	sticky.onclick= deleteSticky; // pro 5
}

function removeStickyFromDOM(key) {
	var sticky = document.getElementById(key); // pro 6
...
function clearStickyNotes() {
	...
	for (var i = stickies.length-1; i >= 0; i--) { // pro 7
		stickyList.removeChild(stickies[i]);
	}

	// reset notes array
	var stickiesArray = getStickiesArray(); // pro 8
}
